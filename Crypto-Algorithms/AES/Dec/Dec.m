...
% key = inputdlg('Enter File Password:','AES128-Decryption');
% end;
% 
...
%% ===pdf test
ASCIIfile=['69';'C4';'E0';'D8';'6A';'7B';'04';'30';'D8';'CD';'B7';'80';'70';'B4';'C5';'5A'];
...
for block=1:(numofFileBlocks)
    ...
    waitbarmessageupdate={['Please wait... (Decrypting) '...
    ...
    fprintf('round[%i].is_box  %s \n',round,dec2hex(cipherBlock)');
    
    % ====== ShitRows ======
    tShiftStart=clock;
    cipherBlock=invShiftRows(cipherBlock);
    tShiftEnd=clock;
    tShiftTotal=tShiftTotal+etime(tShiftEnd,tShiftStart);
    fprintf('round[%i].is_row  %s \n',round,dec2hex(cipherBlock)');
    
       
    % ====== Add Round Key ======
    tAddStart=clock;
    cipherKey=GetBlock(KEY,round+1);
    fprintf('round[%i].ik_sch  %s \n',round,dec2hex(cipherKey)');
    cipherBlock=AddRoundKey(cipherBlock,cipherKey);
    tAddEnd=clock;
    tAddTotal=tAddTotal+etime(tAddEnd,tAddStart);
    ...
    temp=[];
    for word=1:4
    temp1=invMixColumn(Word(cipherBlock,word-1));
    temp=[temp temp1];
    end;
    cipherBlock=temp;
    tMixEnd=clock;
    tMixTotal=tMixTotal+etime(tMixEnd,tMixStart);
    fprintf('round[%i].im_col  %s \n',round,dec2hex(cipherBlock)');

    end;
    
    fprintf('round[10].istart %s \n',dec2hex(cipherBlock)');
    % ===== Sub Byte Round (10)=====
    tSubStart=clock;
    cipherBlock=BlockSubByte(cipherBlock);
    tSubEnd=clock;
    tSubTotal=tSubTotal+etime(tSubEnd,tSubStart);
    fprintf('round[10].is_box %s \n',dec2hex(cipherBlock)');
    ...
    tShiftEnd=clock;
    tShiftTotal=tShiftTotal+etime(tShiftEnd,tShiftStart);
    fprintf('round[10].is_row %s \n',dec2hex(cipherBlock)');
    
    % ====== Add Round Key Round (10) ======
    tAddStart=clock;
    cipherKey=GetBlock(KEY,11);
    fprintf('round[10].ik_sch %s \n',dec2hex(cipherKey)');
    cipherBlock=AddRoundKey(cipherBlock,cipherKey);   
    tAddEnd=clock;
    tAddTotal=tAddTotal+etime(tAddEnd,tAddStart);
    
    fprintf('round[10].ioutput %s \n',dec2hex(cipherBlock)');
...
savefilepath=[pathname 'AES128-' filename(1:end-3)];
...
doneMessage={['File Sucsessfuly Encrypted in ' num2str(totalTime,'%3.2f') ' Seconds '...
    ],['Total SubByte Time: ' num2str(tSubTotal,'%5.2f') ' Seconds '...
    ],['Total ShiftRow Time: ' num2str(tShiftTotal,'%5.2f') ' Seconds '...
    ],['Total MixCol Time: ' num2str(tMixTotal,'%5.2f') ' Seconds '...
    ],['Total AddKey Time: ' num2str(tAddTotal,'%5.2f') ' Seconds '...
